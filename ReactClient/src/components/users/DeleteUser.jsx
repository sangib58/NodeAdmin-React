...
import { deleteSingleUser,deleteCancel,deleteClose } from '../../store/features/userSlice'
...
const DeleteUser = () => {
  ...
  const {deleteId,loading}=useSelector((state)=>({...state.user}))
    
  const deleteConfirm=(e)=>{
    e.stopPropagation()
    dispatch(deleteSingleUser({deleteId}))
    .then((res)=>{
  ...
  return ( 
    <>     
      <div className="fixed top-0 left-0 w-full h-full bg-black/20 z-50">
        <div className="min-h-screen flex justify-center items-center" onClick={(e)=>{if(e.target==e.currentTarget){dispatch(deleteCancel(null))}}}>
          <div className="relative bg-gray-900 w-[400px] h-40 rounded-md p-6 flex flex-col text-white">
            <div className="font-bold text-xl">Are you sure to delete this user?</div>
            ...
            <div className="absolute bottom-3 right-5 space-x-2">
              <button type="button" className="hover:bg-gray-600 rounded p-1 text-md font-bold" onClick={(e)=>{e.stopPropagation();dispatch(deleteCancel(null))}}>Cancel</button>
